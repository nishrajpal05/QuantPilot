...
def run_backtest(
    ...
    Args:
        ...
        df:              OHLCV DataFrame with lowercase columns + DatetimeIndex.
                         Must carry df.attrs["data_source"] and df.attrs["rows_used"].
    ...
    Raises:
        ...
        TimeoutError:  If the strategy takes longer than 30 seconds (Unix).
        RuntimeError:  If the generated code is invalid or crashes.
    """
    # Read data provenance from DataFrame attrs
    data_source = df.attrs.get("data_source", "unknown")
    rows_used = df.attrs.get("rows_used", len(df))

    # Validate data source — refuse sample data in live mode
    if data_source == "sample" and os.getenv("QP_USE_LIVE_DATA", "").lower() in {"1", "true", "yes"}:
        raise RuntimeError(
            "Backtest aborted: data_source='sample' is not allowed when "
            "QP_USE_LIVE_DATA=1. Ensure real OHLCV data was fetched."
        )

    # 1. Run strategy code safely in the sandbox (30 s timeout)
    portfolio = execute_strategy(code, df, initial_capital)
